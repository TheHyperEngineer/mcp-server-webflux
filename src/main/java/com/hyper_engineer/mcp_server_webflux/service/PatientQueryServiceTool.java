...
public class PatientQueryServiceTool {
    ...
    @Tool(
            name = "patientProfileWithDiseasesAndHistory",
    ...
    @Tool(name = "patientMostRecentTest", description = "Patient's most recent test. Returns the latest test record (type and date) for a patient; used for status indicators and due-date logic.")
    ...
    @Tool(name = "patientOverdueForTest", description = "Find patients overdue for a specific test. Identify patients overdue for a named recommended test using medical_tests.recommended_frequency_months.")
    ...
    @Tool(name = "patientByDiseaseCount", description = "Partition patients by number of diseases. Partition patients into single-disease vs multi-disease groups for segmentation and resource planning.")
    ...
    @Tool(name = "patientDiseaseTestMix", description = "Patients with disease and specified test mix. Find patients with a disease who also have at least one of the supplied tests; used for guideline compliance checks.")
    ...
    @Tool(name = "patientCoverage", description = "Coverage of patients with at least one test. Compute percent of patients with at least one test; fast data-quality metric for dashboards.")
    ...
    @Tool(name = "diseaseListPatients", description = "List patients for a disease. List patients diagnosed with a particular disease; base cohort for analysis.")
    ...
    @Tool(name = "diseaseDistribution", description = "Disease distribution across population. Compute counts and percent distribution of diseases across the population for KPI and dashboards.")
    ...
    @Tool(name = "diseaseCompliance", description = "Compliance rates for disease-required tests. For a disease, return its recommended tests and per-test compliance rates vs recommended frequency.")
    ...
    @Tool(name = "diseaseComorbidityPatterns", description = "Comorbidity pair frequencies. Compute pairwise disease co-occurrence frequencies to surface common comorbidities.")
    ...
    @Tool(name = "diseaseRankByPatientCount", description = "Rank diseases by patient count. Rank diseases by number of distinct patients for prioritization and reporting.")
    ...
    @Tool(name = "testPatientsReceiving", description = "Patients receiving a test and counts. List patients who ever received a specific test and how many times for utilization analysis.")
    ...
    @Tool(name = "testUtilizationMonthly", description = "Monthly utilization for a test. Return monthly time-series counts for a test between given dates for capacity planning.")
    ...
    @Tool(name = "testIntervalsBetween", description = "Intervals between repeat tests per patient. Compute per-patient intervals between repeat tests and return median/mean to analyze retesting cadence.")
    ...
    @Tool(name = "testLowUsage", description = "Find tests with low usage. Identify tests with low distinct patient usage relative to recommended frequency for audit and outreach.")
    ...
    @Tool(name = "testUnusuallyFrequent", description = "Flag unusually frequent repeat tests. Flag patients who had the same test more frequently than a threshold (possible redundancy/overuse).")
    ...
    @Tool(name = "utilOverdueAlerts", description = "Generate overdue alerts for tests. Active alert list of patients overdue for disease-specific recommended tests for outreach automation.")
    ...
    @Tool(name = "utilTrendByPeriod", description = "Compute time-series trend for a test or cohort. Generic aggregation tool to compute time-series (monthly/quarterly/yearly) for a test or cohort.")
    ...
    @Tool(name = "utilCohortRetention", description = "Compute cohort retention at intervals. Compute retention of a cohort across follow-up intervals for program evaluation.")
    ...
    @Tool(name = "utilTimeToFirstTest", description = "Compute time from diagnosis to first test. Compute days from diagnosis to first appropriate test for quality metrics and delay analysis.")
    ...
    @Tool(name = "utilDataHealthReport", description = "Generate a data quality snapshot. Comprehensive data-quality snapshot including missing fields, referential integrity, duplicates, and date anomalies.")
    ...
    @Tool(name = "analyticsBuildFeatureTable", description = "Build ML feature table for patients. Build ML-ready per-patient feature vectors (test counts, recency, comorbidity counts) for model training.")
    ...
    @Tool(name = "analyticsSurvivalInputs", description = "Prepare survival model inputs. Prepare time-to-event dataset (patient-level longitudinal features) for survival/risk modeling.")
    ...
    @Tool(name = "analyticsClusterPatients", description = "Cluster patients using feature vectors. Run unsupervised clustering on patient feature vectors and return clusters for segmentation.")
    ...
    @Tool(name = "analyticsPredictMissedTest", description = "Predict patients likely to miss next test. Score patients for probability of missing next recommended test using a trained model or default heuristic.")
    ...
    @Tool(name = "reportKpis", description = "Compute KPI metrics. Compute KPIs like tests per patient/year and percent meeting bundles for dashboards.")
    ...
    @Tool(name = "auditPatientAuditTrail", description = "Patient audit trail for compliance. Produce audit trail of tests and decisions used to determine compliance for a patient (for regulators).")
    ...
    @Tool(name = "healthDetectMalformedRecords", description = "Detect malformed/missing records. Find records missing required fields or with bad formats; useful for pre-ingest checks.")
    ...
    @Tool(name = "healthFindDuplicates", description = "Find duplicate test entries. Find exact duplicate test entries (same patient, same test, same timestamp) to deduplicate.")
    ...
    @Tool(name = "forecastCapacityByTest", description = "Forecast lab capacity per test. Forecast lab capacity needs per test type based on historical utilization and simple growth model.")
    ...
    @Tool(name = "exportPatientList", description = "Export filtered patient list for campaigns. Export patient lists for campaigns with filters and scheduled delivery.")
